...
public class WarnAction {
    public static int warn(Collection<GameProfile> players, String reason, String who, Consumer<Text> responder) {
        JsonObject stamp = ProhibitorCommands.stamp(reason, who);

        for (GameProfile player : players) {
            BackendServer.asyncPost("prohibitor/warn/" + player.getId(), stamp.toString(), (success, response) -> {
