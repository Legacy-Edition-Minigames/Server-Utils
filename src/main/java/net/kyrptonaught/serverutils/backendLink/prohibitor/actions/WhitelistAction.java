...
public class WhitelistAction {
    ...
    public static int whitelist(Collection<GameProfile> players, String reason, String who, Consumer<Text> responder) {
        JsonObject stamp = ProhibitorCommands.stamp(reason, who);

        for (GameProfile player : players) {
            BackendServer.asyncPost("prohibitor/whitelist/add/" + player.getId(), stamp.toString(), (success, response) -> {
