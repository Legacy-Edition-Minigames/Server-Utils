...
public class SkinBanAction {
    ...
    public static int skinBan(Collection<GameProfile> players, String reason, String who, Consumer<Text> responder) {
        JsonObject stamp = ProhibitorCommands.stamp(reason, who);

        for (GameProfile player : players) {
            BackendServer.asyncPost("prohibitor/ban/skin/" + player.getId(), stamp.toString(), (success, response) -> {
