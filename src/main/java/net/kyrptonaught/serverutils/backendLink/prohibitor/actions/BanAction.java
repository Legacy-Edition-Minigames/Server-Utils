...
public class BanAction {
    public static int permBan(Collection<GameProfile> players, String reason, String who, Consumer<Text> responder) {
        JsonObject stamp = ProhibitorCommands.stamp(reason, who);

        for (GameProfile player : players) {
            BackendServer.asyncPost("prohibitor/ban/perm/" + player.getId(), stamp.toString(), (success, response) -> {
    ...
    public static int tempBan(Collection<GameProfile> players, String reason, String who, ChronoUnit unit, int durationTime, Consumer<Text> responder) {
        JsonObject stamp = ProhibitorCommands.stamp(reason, who);
        stamp.addProperty("duration_time", durationTime);
        stamp.addProperty("duration_type", unit.ordinal());

        for (GameProfile player : players) {
            BackendServer.asyncPost("prohibitor/ban/temp/" + player.getId(), stamp.toString(), (success, response) -> {
                if (!success) responder.accept(Text.literal("Error Punishing Player"));
            });
        }
